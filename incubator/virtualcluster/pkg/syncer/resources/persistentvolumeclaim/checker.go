...
func (c *controller) PatrollerDo() {
	...
	for _, clusterName := range clusterNames {
		...
		go func(clusterName string) {
			defer wg.Done()
			c.checkPVCOfTenantCluster(ctx, clusterName)
		}(clusterName)
	}
	wg.Wait()
	...
	for _, pPVC := range pPVCs {
		...
		err := c.MultiClusterController.Get(ctx, clusterName, objectKey, vPVC)
		...
		if shouldDelete {
			...
			if err = c.pvcClient.PersistentVolumeClaims(pPVC.Namespace).Delete(ctx, pPVC.Name, *deleteOptions); err != nil {
...
func (c *controller) checkPVCOfTenantCluster(ctx context.Context, clusterName string) {
	pvcList := &v1.PersistentVolumeClaimList{}
	if err := c.MultiClusterController.List(ctx, clusterName, pvcList); err != nil {
