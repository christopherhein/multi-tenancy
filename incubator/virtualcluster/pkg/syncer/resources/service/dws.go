...
func (c *controller) Reconcile(request reconciler.Request) (reconciler.Result, error) {
	...
	if err := c.MultiClusterController.Get(ctx, request.ClusterName, request.NamespacedName, vService); err != nil {
		if !errors.IsNotFound(err) {
			return reconciler.Result{Requeue: true}, err
		}
		vExists = false
	...
	if vExists && !pExists {
		err := c.reconcileServiceCreate(ctx, request.ClusterName, targetNamespace, request.UID, vService)
		if err != nil {
		...
		err := c.reconcileServiceRemove(ctx, request.ClusterName, targetNamespace, request.UID, request.Name, pService)
		if err != nil {
			klog.Errorf("failed reconcile service %s/%s DELETE of cluster %s %v", request.Namespace, request.Name, request.ClusterName, err)
			return reconciler.Result{Requeue: true}, err
		}
	} else if vExists && pExists {
		err := c.reconcileServiceUpdate(ctx, request.ClusterName, targetNamespace, request.UID, pService, vService)
		if err != nil {
...
func (c *controller) reconcileServiceCreate(ctx context.Context, clusterName, targetNamespace, requestUID string, service *v1.Service) error {
	...
	pService, err = c.serviceClient.Services(targetNamespace).Create(ctx, pService, metav1.CreateOptions{})
...
func (c *controller) reconcileServiceUpdate(ctx context.Context, clusterName, targetNamespace, requestUID string, pService, vService *v1.Service) error {
	if pService.Annotations[constants.LabelUID] != requestUID {
	...
	if updated != nil {
		_, err = c.serviceClient.Services(targetNamespace).Update(ctx, updated, metav1.UpdateOptions{})
		if err != nil {
			return err
		}
	}
...
func (c *controller) reconcileServiceRemove(ctx context.Context, clusterName, targetNamespace, requestUID, name string, pService *v1.Service) error {
	if pService.Annotations[constants.LabelUID] != requestUID {
	...
	err := c.serviceClient.Services(targetNamespace).Delete(ctx, name, *opts)
	if errors.IsNotFound(err) {
