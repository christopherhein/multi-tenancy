...
func (c *controller) BackPopulate(key string) error {
	...
	if err := c.MultiClusterController.Get(ctx, clusterName, objectKey, vPod); err != nil {
		if errors.IsNotFound(err) {
			return nil
		}
	...
	if vPod.Spec.NodeName == "" {
		n, err := c.client.Nodes().Get(ctx, pPod.Spec.NodeName, metav1.GetOptions{})
		if err != nil {
		...
		if err := c.MultiClusterController.Get(ctx, clusterName, types.NamespacedName{Namespace: "", Name: n.GetName()}, &v1.Node{}); err != nil {
			...
			if err := tenantClient.Create(ctx, vn); err != nil && !errors.IsAlreadyExists(err) {
				return fmt.Errorf("failed to create virtual node %s in cluster %s with err: %v", pPod.Spec.NodeName, clusterName, err)
			}
		}
		cfg, err := c.MultiClusterController.GetClusterRestConfig(clusterName)
		if err != nil {
			return fmt.Errorf("failed to get clusters restconfig")
		}

		clientset, err := kubernetes.NewForConfig(cfg)
		if err != nil {
			return fmt.Errorf("failed to create clientset for subresources")
		}

		err = clientset.CoreV1().Pods(vPod.Namespace).Bind(ctx, &v1.Binding{
		...
		vPod := &v1.Pod{}
		if err = tenantClient.Get(ctx, types.NamespacedName{Namespace: vPod.Namespace, Name: vPod.Name}, vPod); err != nil {
		...
		if err := c.MultiClusterController.Get(ctx, clusterName, types.NamespacedName{Namespace: "", Name: vPod.Spec.NodeName}, &v1.Node{}); err != nil {
			if errors.IsNotFound(err) {
	...
	if updatedMeta != nil {
		...
		if err = tenantClient.Update(ctx, newPod); err != nil {
	...
	if newStatus := conversion.Equality(c.Config, vc).CheckUWPodStatusEquality(pPod, vPod); newStatus != nil {
		if newPod == nil {
			...
			objectKey := types.NamespacedName{Namespace: vPod.Namespace, Name: vPod.Name}
			if err = tenantClient.Get(ctx, objectKey, newPod); err != nil {
		...
		if err = tenantClient.Status().Update(ctx, newPod); err != nil {
	...
	if pPod.DeletionTimestamp != nil {
		if vPod.DeletionTimestamp == nil {
			...
			deleteOptions := &client.DeleteOptions{GracePeriodSeconds: pPod.DeletionGracePeriodSeconds}
